	...
	store.getState().counters.forEach((counter) => {
		newCounterEl.innerHTML = `
			<div class="p-4 h-auto flex flex-col items-center justify-center space-y-5 bg-white rounded shadow">
				...
				<div id="${counter.counterID + 1}" class="text-2xl font-semibold">${counter.value}</div>

				<div class="flex space-x-3">
					<button 
						id="increment-${counter.counterID}"
						onclick="increment('${counter.counterID + 1}', '${counter.incrementBy}')"
    ...
    newCounter.innerHTML = `
		<div class="p-4 h-auto flex flex-col items-center justify-center space-y-5 bg-white rounded shadow">
			...
			<div class="flex space-x-3">
				...
				<button 
					id="decrement-${initialState.counters + 1}"
					onclick="decrement('${counter.counterID + 1}', '${counter.decrementBy}')"
					class="bg-red-400 text-white px-3 py-2 rounded shadow">
					Decrement By ${counter.decrementBy}
				</button>
			</div>
